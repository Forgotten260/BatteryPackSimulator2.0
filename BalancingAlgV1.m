...
% Algorithm Parameters:
    ...
    CentralBuffer = 0.005; % If cell voltages are within this zone either side then they won't be balanced. 
    
    MaxCellsToCharge = 4;  %Ie cells in the secondary for discharge
    MaxCellsToDischarge = 4; %Ie cells in the secondary for charge

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Extract OCV and Cell ID Values
%                                                                                                                                                                                                                    

    V_TerminalArray = zeros(1,SeriesCells);
    cellIDs = zeros(1,SeriesCells);
    SortedOCVs = zeros(1,SeriesCells);
    SortedCellIDs = zeros(1,SeriesCells);
    
    for i = 1:SeriesCells
        V_TerminalArray(i) = CellArray(i).V_Terminal;
        cellIDs(i) = CellArray(i).CellSeriesID;
    end
    
    [SortedOCVs, SortedCellIDs] = bubbleSortOCVsWithCellIDs(V_TerminalArray, cellIDs,PrevSortedCellIDs);
...
%Filter Cells that are within buffer zone of the reference voltage.
    ...
    for i = 1:MaxCellsToCharge
        ...
        tmp = RefVoltage - V_TerminalArray(CellsToCharge(i));
    ...
    for i = 1:MaxCellsToDischarge
        if V_TerminalArray(CellsToDischarge(i)) - RefVoltage <CentralBuffer
...
%can check with V_T calculation. if not can then simiply go past. 
    ...
    for i = 1:MaxCellsToDischarge
        if(CellsToDischarge(i)~=0)
            % if (V_TerminalArray(CellsToDischarge(i))> UpperBuffer )||( V_TerminalArray(CellsToDischarge(i)) < LowerBuffer )
                if ( V_TerminalArray(CellsToDischarge(i)) < LowerBuffer )
                CellsToDischarge(i)=0;
                % CellsToDischarge(TotalCellsToBalance/2)= CellsToDischarge(TotalCellsToBalance/2)-1;
            end
        end
    ...
    for i = 1:MaxCellsToCharge
        if (CellsToCharge(i)~=0)
            % if (V_TerminalArray(CellsToCharge(i))> UpperBuffer )||( V_TerminalArray(CellsToCharge(i)) < LowerBuffer )
            if (V_TerminalArray(CellsToCharge(i))> UpperBuffer )
                CellsToCharge(i)=0;
                % CellsToDischarge(TotalCellsToBalance/2)= CellsToDischarge(TotalCellsToBalance/2)-1;
            end
        end
