...
%in here
        ...
        TotalCapacity =         round((randn(SeriesCells,1)/3*CapacityStdDev+NominalCapacity)*ParallelCells,4) ;
        InternalResistance =    round((randn(SeriesCells,1)/3*IntResistanceStdDev+IntResistanceNominal)/ParallelCells,4);
        CurrentCapacity=        SOC_Start *TotalCapacity;
        Rp1 =                   round((randn(SeriesCells,1)/3*Rp1Std+Rp1Nom)/ParallelCells,4);
        Rp2 =                   round((randn(SeriesCells,1)/3*Rp2Std+Rp2Nom)/ParallelCells,4);
        Cp1 =                   round((randn(SeriesCells,1)/3*Cp1Std+Cp1Nom)*ParallelCells,4); % Capacitance is multipled by parallel components.
        Cp2 =                   round((randn(SeriesCells,1)/3*Cp2Std+Cp2Nom)*ParallelCells,4);
